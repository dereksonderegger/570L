...
By default, the `geom_box()` just counts the number of cases and displays how many observations were in each factor level. This behavior can modified using the `stat=` option. In particular, if I have a data frame that I have already summarized, `stat=identity` will allow you to set the height of the bar.
