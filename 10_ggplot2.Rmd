...
By default, the `geom_box()` just counts the number of cases and displays how many observations were in each class. This behavior can modified using the `stat=` option. In particular, if I have a data frame that I have already summarized, `stat=identity` will allow you to set the height of the bar.
...
Just as `geom_bar` by default calculated the number of observations in each level of my factor of interest (which was `class`, `geom_histogram` breaks up my x-axis into distinct bins (by default in uses 30 bins), and then counts how many observations fall into each bin, and displys the number as a bar. To change the number of bins, we could either tell it the number of bins (e.g. `bins=20`) or the width of each bin (e.g. `binwidth=4`).
...
If you want to instead pick a color palette and let ggplot pick the colors to be farthest apart based on the number of factor levels, you can use `scale_color_manual` and then have the values chosen by one of the palette functions where you just have to tell it how many levels you have.

```{r}
library(colorspace)   # these two packages have some decent 
library(grDevices)    # color palettes.

rainbow(6)   # if you have six factor levels, what colors should we use 
...
If we don't want to specify the colors manually we can, as usual, specify the color palete. The `gradientn` functions allow us to specify a large numbers intermediate colors.
```{r}
ggplot( Eden, aes(x=x, y=y, fill=elevation)) +
  geom_tile() +
...
### Zooming in/out
It is often important to be able to force the graph to have a particular range in either the x-axis or the y-axis.  Given a particular range of intest, there are two ways that we could this:
